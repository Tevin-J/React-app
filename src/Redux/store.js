...
let store = {
    ...
    _callSubscriber () { /*заглушка, которая потом попадает в другие ф-и store.js. необходимо было ее здесь
создать, потому что ее нельзя импортировать из index.js, чтоб не было циклической зависимости*/
    ...
    subscribe (observer) { /*нам в store.js нужна subscribe для того чтоб перерисовывать дерево
